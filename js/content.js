...
// Prevent duplicate class declaration on page refresh
if (typeof window.VideoDetector !== 'undefined') {
	console.log('Video Download Helper: Content script already initialized, skipping');
} else {
	...
	class VideoDetector {
		...
		extractQualityFromUrl(url) {
			...
			// Fallback: common quality indicators
			...
			for (const [indicator, quality] of Object.entries(qualityIndicators)) {
				if (u.toLowerCase().includes(indicator)) {
					return quality;
				}
			}
		...
		setupChineseVideoSiteDetection() {
			...
			const intervalId = setInterval(checkForVideos, 3000);
...
} // End of duplicate protection check
